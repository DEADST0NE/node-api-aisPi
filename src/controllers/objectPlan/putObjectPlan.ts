...
const putObjectPlan = (req: Request, res: Response) => { 
  ...
  const objectValue = req.body.objectValue as undefined | string;
  ...
  if(objectId && objectName && objectType && kbk && Number(planYearBudget) >= 0 && Number(planYearBudget1) >= 0 && Number(planYearBudget2) >= 0) {
