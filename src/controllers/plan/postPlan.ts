...
const postPlan = (req: Request, res: Response) => { 
  const organizarionId = req.body.organizarionId as undefined | string;
  ...
  if(organizarionId && yearStart && yearStop && codeOKEI && planType && namePlan) {
    prisma.d_plan.create({
      data: {
        s_organization: {
          connect: {
            id: String(organizarionId),
          }
        },
        name: namePlan,
